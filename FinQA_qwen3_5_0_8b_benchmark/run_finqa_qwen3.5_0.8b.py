...
class QwenPrompter:
    def __init__(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3.5-0.8B", trust_remote_code=True)
        self.model = AutoModelForCausalLM.from_pretrained("Qwen/Qwen3.5-0.8B", trust_remote_code=True, torch_dtype=torch.float16).to(self.device).eval()
    ...
    def run_all(self, eval_data, retriever):
        ...
        for method in ["cloze_simple", "pet_style", "cloze_rag", "prefix_zeroshot", "prefix_rag", "prefix_cot", "decompose_rag"]:
            for item in tqdm(eval_data, desc=method):
                ctx = item["context"][:800]; q = item["question"]; target = ""
                if method == "cloze_simple": p = f"Context: {ctx}\nQuestion: {q}\nThe answer is "
                elif method == "pet_style": p = f"{ctx}\nBased on table, the answer to '{q}' is "
...
print("\n--- Qwen3.5-0.8B Benchmark ---")
for method in out_df["method"].unique():
    acc = out_df[out_df["method"] == method]["correct"].mean()
    print(f"[{method}] Acc (EM-relaxed): {acc:.4f}")
out_df.to_csv("/home/tienpv16/Documents/Research/project_SML/SML_master_R5/FinQA_qwen3_5_0_8b_benchmark/results_finqa.csv", index=False)
